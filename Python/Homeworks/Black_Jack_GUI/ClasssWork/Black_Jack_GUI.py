...
class Game:
    ...
    def __init__(self):
        ...
        btn_new = Button(main_window,text="New Game", font="Arial 15", command=self.new_game)
        ...
        btn_done = Button(main_window, text="   Done   ", font="Arial 15", bg=colors[0], fg=colors[1], command=self.done)
        ...
        #  Testing!
        for i in range(5):
            colors = self.random_color(i)
            Button(main_window, text="TestBtn"+str(i), font="Arial 15", bg=colors[0], fg=colors[1]).\
                grid(row=i+2, column=1)


        for c in range(0, 6):
            self.draw_card(self.deck.cards.pop(), c*120 , 400)
        #  <Testing!"""
    ...
    def update_score(self, p):
        ...
        self.game_board.itemconfig(self.player_score_text, text=self.players[p][1])

    def check_wins(self, final = False):
        if not final:
            if self.players[int(self.turn)][1] == 21:
                print(self.players[int(self.turn)][2] + ' wins')
                self.game_over(1)
                return 1
            elif self.players[int(self.turn)][1] > 21:
                print(self.players[int(self.turn)][2] + ' busts')
                self.game_over(0)
                return 0
            return 0
        else:
            if self.players[0][1] == self.players[1][1]:
                ...
                print('{} wins '.format(self.players[0][2]))
                self.game_over(0)
                return 1
            else:
                print('{} wins '.format(self.players[1][2]))
                self.game_over(0)
                return 1
    ...
    def hit(self, t=0):
        ...
        if t == 0:
            ...
            self.draw_card(self.players[t][0][-1], self.card_player_pointer_x, 20)
            self.card_dealer_pointer_x += self.card_size[0] + 20
        ...
        if self.players[t][1] > 21:
            print('{} busts '.format(self.players[t][2]))
            self.game_over(0)
            return 1
    ...
    def game_over(self, win):
        print("Heeeeeeeeeeeeeeeeeyyyyyyyyyyyy")
        self.btn_hit.config(state="disabled")
        if win:
            self.update_money(win)
            self.game_board.itemconfig(self.player_score_text, text="Player wins")
        else:
            self.update_money(win)
            self.game_board.itemconfig(self.player_score_text, text="Dealer wins")


    def update_money(self, win):
        if win:
            self.money += 200
            self.game_board.itemconfig(self.lbl_money, text=str(self.money))
        else:
            self.money -= 200
            self.game_board.itemconfig(self.lbl_money, text=str(self.money))
